...
for i = 1:nSteps
    ...
    for k = 1:Reg.n
        ...
        if bc{k}.NumS > 0
            for source = 1:bc{k}.NumS
            Einc = bc{k}.s(source).fct(yC{k},i,t,bc{k}.s(source).paras);
            Eincp = bc{k}.s(source).fct(yC{k},i,t-ds/c_c/2,bc{k}.s(source).paras);
            
            Hy{k}(bc{k}.s(source).xpos,:) = Hy{k}(bc{k}.s(source).xpos,:) + dt/ds/c_mu_0*Einc;
            Ez{k}(bc{k}.s(source).xpos-1,:) = Ez{k}(bc{k}.s(source).xpos-1,:) - dt/ds/c_eps_0/c_eta_0*Eincp;
            end
        end
